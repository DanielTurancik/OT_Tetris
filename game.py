...
class Game:
    ...
    def get_random_block(self):
        completed = self.grid.count_full_rows()
        if completed > 1:
            return SpecialBlock()
    ...
    def lock_block(self):
        ...
        if isinstance(self.current_block, SpecialBlock):
            columns_to_clear = set(position.col for position in tiles)
            for col in columns_to_clear:
                self.clear_column(col)
                self.grid.play_sound()
            self.current_block = self.next_block
            self.next_block = self.get_random_block()
            self.current_block.move(-self.current_block.get_cell_positions()[0].row, 0)
        else:
            self.current_block = self.next_block
            self.next_block = self.get_random_block()
        cleared_rows = self.grid.clear_full_rows(self.screen)
    ...
    def clear_column(self, col):
        self.grid.animate_column_clear(self.screen, col)
